...
use Mralston\LaravelPasswordlessLogin\Exceptions\InvalidSignatureException;
use Mralston\LaravelPasswordlessLogin\Exceptions\ExpiredSignatureException;
use Mralston\LaravelPasswordlessLogin\LoginUrl;
use Mralston\LaravelPasswordlessLogin\Models\Models\User as ModelUser;
use Mralston\LaravelPasswordlessLogin\Models\User;
...
class SignedUrlTest extends TestCase
{
    ...
    public function setUp(): void
    {
        ...
        list($route, $uid) = explode(
            "/",
            ltrim(parse_url($this->url)["path"], "/")
        );
        $expires = explode(
            "=",
            explode("&", explode("?", $this->url)[1])[0]
        )[1];
    ...
    public function can_create_default_signed_login_url()
    {
        ...
        $this->assertEquals(
            config("laravel-passwordless-login.login_route_name"),
            $this->route
        );
    }
    ...
    public function allows_override_of_post_login_redirect()
    {
        ...
        $generator->setRedirectUrl(
            "/laravel_passwordless_login_redirect_overridden_route"
        );
        $this->url = $generator->generate();
        $response = $this->followingRedirects()->get($this->url);
